...
*/
  ...
  encoder_down = 100; //2580;                              // Encoder reading when door is down (Previous test value = 1085)
...
void loop()
{
  ...
  if (initialize_counter==0) {
    ...
    delay(1000);
    // If initial door position is not correct then open door A
    if (doorA_init_desired != doorA_position) {
      change_doorA();
    }
    // If initial door position is not correct then open door B
    else if (doorB_init_desired != doorB_position) {
      change_doorB();
    }
