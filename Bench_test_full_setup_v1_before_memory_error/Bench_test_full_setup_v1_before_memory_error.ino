...
void setup()
{
  ...
  motor_off_encoder = 0;
  door_trigger = 0;       // Used to make doors move on fixed schedule
...
void loop()
{
  ...
  if ((door_trigger==0) && (motor_off_encoder==0)) {
    time2 = millis();   // Set timer after door has been triggered
    door_trigger=1;     // Denotes that countdown has started for next door to move
  } 
  ...
  if ((motor_off_encoder==0) && ((millis()-time2)>=door_timer) && doorA_has_moved==1 && (1==0)) {
