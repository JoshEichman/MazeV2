 ...
 *   NOTES: 
 *   Works as is to control 2 motors based on SQL input.
 ...
 *   Fixed issues with stopping after every 180s by deleting cur_mem for each SQL call (my guess is that doesn't allow the cursors to build up in memory)
...
char SELECT_SQL_MAC[] = "SELECT Cell FROM MazeV2mac.ESP_to_MAC WHERE MAC=";

char SQL_cell_init[] = " WHERE Cell=";
char SQL_A[16];
char SQL_B[16];
char cstr[16];
...
void setup()
{
  ...
  */
  ...
  door_timer = 2000;                                // (UNUSED) Time to wait between door changes (ms) 
  door_timeout = 18000;                             // Max time motors are allowed to run (ms)
  ...
  reset_door_frequency = 4;                         // Reset door by running to max position and resetting encoder after this many door lifting events
...
{  
  ...
  if (initialize_counter==0) {
    if (1==1) {
      ...
      for(int i = 0; i < 6; i++) {
        ...
        itoa(mac[i], &address[strlen(address)], 16);
        if(i<5)address[strlen(address)]=':';
        // // Serial.print(" ");
      }      
      // //Serial.println(address);

      char SELECT_SQL_MAC_2[50];
      strcpy(SELECT_SQL_MAC_2,SELECT_SQL_MAC);
      strcat(SELECT_SQL_MAC_2,"'");
      strcat(SELECT_SQL_MAC_2,address);
      strcat(SELECT_SQL_MAC_2,"'");   // Serial.println(SELECT_SQL_MAC_2);

      // Pull correct cell based on MAC address
      MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn);    // You can leave it in each iteration, it runs quickly
      // Execute the query   
      cur_mem->execute(SELECT_SQL_MAC_2);
      ...
      do {
        ...
        if (row != NULL) {
          Cell_val = atoi(row->values[0]);            
        }
      } while (row != NULL); 
      delete cur_mem; 

      strcpy(SQL_A,SQL_cell_init);
      strcat(SQL_A,itoa(Cell_val+0, cstr, 10)); //       Serial.println(SQL_A);      
      strcpy(SQL_B,SQL_cell_init);
      strcat(SQL_B,itoa(Cell_val+1, cstr, 10)); //       Serial.println(SQL_B);
      
      Serial.print("Selected Cell "), Serial.print(Cell_val), Serial.print(" and Cell "), Serial.println(Cell_val+1);
    }
    if (1==1) { // Pull initial values (make sure no values that will be selected start with Null)
      MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn);    // You can leave it in each iteration, it runs quickly
      char SELECT_SQL_DESIRED_2[30];
      strcpy(SELECT_SQL_DESIRED_2,SELECT_SQL_DESIRED);
      strcat(SELECT_SQL_DESIRED_2,SQL_A);  //  Serial.println(SELECT_SQL_DESIRED_2);
      // Execute the query   
    ...
    if (1==1) {
      ...
      strcat(SELECT_SQL_ACTUAL_2,SQL_A);  //  Serial.println(SELECT_SQL_ACTUAL_2);
    ...
    if (1==1) {
      ...
      char SELECT_SQL_DESIRED_2[30];
      strcpy(SELECT_SQL_DESIRED_2,SELECT_SQL_DESIRED);
      strcat(SELECT_SQL_DESIRED_2,SQL_B);  //  Serial.println(SELECT_SQL_DESIRED_2);
      // Execute the query   
      cur_mem->execute(SELECT_SQL_DESIRED_2);
      column_names *cols = cur_mem->get_columns();
      row_values *row = NULL;
    ...
    if (1==1) {
      ...
      char SELECT_SQL_ACTUAL_2[30];
      strcpy(SELECT_SQL_ACTUAL_2,SELECT_SQL_ACTUAL);
      strcat(SELECT_SQL_ACTUAL_2,SQL_B);  //  Serial.println(SELECT_SQL_ACTUAL_2);
      cur_mem->execute(SELECT_SQL_ACTUAL_2);
      column_names *cols = cur_mem->get_columns();
      row_values *row = NULL;
      // Prints single row value (good reference for SELECT: https://github.com/ChuckBell/MySQL_Connector_Arduino/issues/19)
  ...
  // Debug SQL calls
  // Serial.print("SQL_A = "), Serial.print(SQL_A), Serial.print(" and SQL_B = "), Serial.println(SQL_B);
  ...
  // Below section doesn't work (need to check status before each DB call or something else more clever)
