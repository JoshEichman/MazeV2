...
void setup()
{
  ...
  doorA_position = 1;     // 0=up, 1=down
  doorB_position = 1;     // 0=up, 1=down  
  ...
  encoder_down = 600;                               // Encoder reading when door is down (Previous test value = 1085)
  door_timer = 10000;                               // Time to wait between door changes (ms) 
  initialize_counter = 0;                           // Used to initialize the door from closed to open}
...
void loop()
{
  ...
  // Only run this once (possibly put in setup, not sure if encoders work without their initialization in the few lines above)
  // For now use this migrate to using the databases last known position
  if (initialize_counter==0) {
    ...
    while (doorA_position == 1) {
      delay(2);
      if (knobA.read() >= encoder_down) {
        doorA_position = 0;             // Change to show new door position
        doorA_previous_position = abs(doorA_position-1); // Reset door previous position 
      }
    }
  ...
  if ((millis()-time2)>=door_timer) {
    ...
    if (doorA_position == 0) {        // If its down then take it up
      ...
      while (knobA.read() < 10 || (millis()-time2)<door_timer+9000) {
        delay(5);
        if (knobA.read() > 10) {
          Serial.println(knobA.read());    // Turns off motor if the door was stopped manually or automatically      
        }
      }
      */
      while (doorA_position == 0) {
        delay(2);
        if ((knobA.read() <= -1) || (millis()-time2)>=door_timer+9000) {
          doorA_position = 1;             // Change to show new door position
        }
      }
      Serial.println("Motor Off");    // Turns off motor if the door was stopped manually or automatically
      ...
      while (doorA_position == 1) {
        delay(2);
        if ((knobA.read() >= encoder_down) || (millis()-time2)>=door_timer+4000) {
          doorA_position = 0;             // Change to show new door position
        }
      }
      Serial.println("Motor Off");    // Turns off motor if the door was stopped manually or automatically
